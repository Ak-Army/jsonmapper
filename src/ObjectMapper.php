...
/**
 * Automatically map JSON/XML structures into objects.
 */
class ObjectMapper
{
    /**
    ...
    public function map($json, $object)
    {
        if ($this->bEnforceMapType && !is_object($json)) {
            throw new \InvalidArgumentException(
                'ObjectMapper::map() requires first argument to be an object'
                .', '.gettype($json).' given.'
            );
        }
        if (!is_object($object)) {
            throw new \InvalidArgumentException(
                'ObjectMapper::map() requires second argument to be an object'
                .', '.gettype($object).' given.'
            );
        }

        if ($json instanceof \SimpleXMLElement) {
            $json = $this->xmlToObj($json);
        }
        ...
        foreach ($json as $key => $jvalue) {
            ...
            if (!$hasProperty) {
                ...
                $this->log('info', 'Property '.$key.' does not exist in '.$strClassName);
                continue;
            }
            ...
            if ($accessor === null) {
                ...
                $this->log('info', 'Property '.$key.' has no public setter method in '.$strClassName);
                continue;
            }
            ...
            if ($type === null || $type === 'mixed') {
                ...
                if ($type == 'bool') {
                    ...
                    if (is_object($first)) {
                        foreach ($jvalue as $jkey => $value) {
                            $jvalue[$jkey] = (array) $value;
                        }
                    }
                } else {
                    settype($jvalue, $type);
    ...
    protected function checkMissingData($providedProperties, \ReflectionClass $rc)
    {
        foreach ($rc->getProperties() as $property) {
            ...
            if (isset($annotations['required'])
                ...
                $this->log('info', 'Required property "'.$property->name.'" of class '.$rc->getName().' is missing in JSON data');
            }
        }
    }
    ...
    public function mapArray($json, $array, $class = null)
    {
        foreach ($json as $key => $jvalue) {
            if ($class === null) {
                $array[$key] = $jvalue;
            } elseif (!is_object($jvalue)) {
                $array[] = $this->map($json, new $class());
                break;
            } elseif ($this->isFlatType(gettype($jvalue))) {
                //use constructor parameter if we have a class
                // but only a flat type (i.e. string, int)
